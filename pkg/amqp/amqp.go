...
func NewServeMux(addr string, tag string, exchanges map[string]config.Exchange, keychain map[string]eventapi.Validator) *ServeMux {
	return &ServeMux{
		addr:      addr,
		tag:       tag,
		exchanges: exchanges,
...
func (mux *ServeMux) declareQueue(channel *amqp.Channel, routingKey string, exchange string) (*amqp.Queue, error) {
	queueName := fmt.Sprintf("mailservice.%s.consumer", routingKey)
